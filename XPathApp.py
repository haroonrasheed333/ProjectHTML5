...
@app.route("/initialcall", methods=['POST'])
def initialcall():
    outputjson = dict()
...
@app.route("/queryxml", methods=['POST'])
def queryxml():

    browser = request.json['browser_name']
    feature = request.json['feature_name']
    app.logger.debug(browser)
    ...
    #return jsonify(**outputsd)
    return json.dumps(outputsd)
...
def main():
    ...
    #if search_spec_for_browser == True:
